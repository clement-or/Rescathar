...
func generate_random_path(world_start):
	var x_tile
	var y_tile
	var cond = true
	while cond:
		randomize()
		x_tile = randi()%int(map_size.x)
		y_tile = randi()%int(map_size.y)
		
		if not (Vector2(x_tile, y_tile) in obstacles):
			cond = false
	
	self.path_start_position = world_to_map(world_start)
	self.path_end_position = Vector2(x_tile, y_tile)
	_recalculate_path()
	var path_world = []
	for point in _point_path:
		var point_world = map_to_world(Vector2(point.x, point.y)) + _half_cell_size
